...
from RAW_HYP import TRAIN_DATA_OUTPUT, TEST_DATA_OUTPUT
...
def trainScalers(filenames):
    ...
    for i in range(0, len(filenames)):
        print(os.path.exists(filenames[i]))
        if os.path.exists(filenames[i]):
            hf = SD(filenames[i], SDC.READ)

            print(os.path.basename(filenames[i]))

            r = hf.select(os.path.basename(filenames[i]))
            dat = r.get()
...
def test_generic():
	...
	#	tmp, coord = format_data(trn[0]["Output"][i])
	#	print(tmp.shape)
	...
	for n in range(len(PERPLEXITY) -1 , -1, -1):
		...
		for l in range(len(td)-1, -1, -1):
			for j in range(len(td[l]["Output"])):	
				...
				indices = np.random.choice(test_data.shape[0], size=int(test_data.shape[0]*0.15), replace=False)
...
def format_data(fname, clust):
    ...
    if os.path.exists(fname):
        ...
        r = hf.select(os.path.basename(fname))
        dat = r.get()
        dat = dat.astype(np.float32)
        ...
        for n in range(dat.shape[1]):
            subd = dat[:,n,:]
            subd[np.where(subd > -99999)] = scalers[n].transform(subd[np.where(subd > -99999)].reshape(-1, 1)).reshape(-1)
            print("SCALED STATS", subd[np.where(subd > -99999)].min(), subd[np.where(subd > -99999)].max(), subd[np.where(subd > -99999)].mean(), subd[np.where(subd > -99999)].std())
            dat[:,n,:] = subd
        ...
        dataRet = []
        dataCoord = []
        for j in range(1, dat.shape[0]-1):
            for k in range(1, dat.shape[2]-1):
                sub_data_total = []
                for c in range(0, dat.shape[1]):
                    sub_data = dat[j-1:j+2,c,k-1:k+2]
    ...
    return np.array(dataRet).astype(np.float32), np.array(dataCoord)
