...
from MISR_MODIS_OUTPUT import TRAIN_DATA_OUTPUT, TEST_DATA_OUTPUT
...
def trainScalers(filenames):
    ...
    for i in range(0, len(filenames)):
        if os.path.exists(filenames[i]):
            dat = np.load(filenames[i])[0].astype(np.float32)
            data.append(dat)
            print(dat.shape)
...
def test_generic():
	...
	for i in range(len(trn[0]["Output"])):
		tmp = np.load(trn[0]["Output"][i])[0].astype(np.float32)
	...
	for n in range(len(PERPLEXITY) -1 , -1, -1):
		...
		for l in range(len(td)-1, -1, -1):
			for j in range(len(td[l]["Output"])):	
				test_data = np.load(td[l]["Output"][j])[0].astype(np.float32)
				for n in range(test_data.shape[1]):
					subd = test_data[:,n]
					subd = scalers[n].transform(subd.reshape(-1,1)).reshape(-1)
					test_data[:,n] = subd

				test_coord =np.load(td[l]["Coord"][j])[0].astype(np.int32)
				...
				test_coord = test_coord[indices]
				print(test_data.shape, clust_data.shape, test_coord.shape)


				print(test_coord.shape, test_data.shape, clust_data.shape)	
