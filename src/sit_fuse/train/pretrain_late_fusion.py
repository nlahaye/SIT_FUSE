...
from sit_fuse.models.encoders.late_fusion import Late_Fusion_DBN_PL
...
def load_encoders(yml_conf, datasets, conv=False):
    ...
    for i in range(len(encoder_confs)):
        ...
        decay = tuple(yml_conf_tmp["encoder"]["training"]["weight_decay"])
        nesterov_accel = tuple(yml_conf_tmp["encoder"]["training"]["nesterov_accel"])

        accelerator = yml_conf_tmp["encoder"]["training"]["accelerator"]
        ...
        if not conv:
            ...
            visible_shape = yml_conf_tmp["data"]["tile_size"]
            number_channel = yml_conf_tmp["data"]["number_channels"]
