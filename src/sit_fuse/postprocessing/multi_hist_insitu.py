...
def main(yml_fpath):
    ...
    #Run 
    start_date = datetime.datetime.strptime( yml_conf['start_date'], '%Y-%m-%d').date()#.replace(tzinfo=datetime.timezone.utc)#.tz_localize(None)
    end_date = datetime.datetime.strptime( yml_conf['end_date'], '%Y-%m-%d').date() #.replace(tzinfo=datetime.timezone.utc) #.tz_localize(None)
