...
from sit_fuse.pipelines.training.emit_preprocess_and_train_constants import *
...
def find_train_test_fpaths(yml_conf, config_dict):
    ...
    train_grid_inds = yml_conf["train_timestamps"]

    file_re = yml_conf["file_re"]
 
    fnames = glob.glob(os.path.join(yml_conf["input_dir"], yml_conf["input_glob"]))
