...
def fuse_data(yml_conf):
    ...
    for i in range(len(lo_filenames):
        lo_dat = read_func_lo(lo_filenames[i], **read_func_kwargs_lo).astype(np.float64)
        ...
        source_def_lo = geometry.SwathDefinition(lons=lo_geo[tuple(slc_lon_lo)], lats=lo_geo[tuple(slc_lat_lo)])
        ...
        lo_dat = np.ma.masked_where((lo_dat < valid_max_lo) | (lo_dat > valid_max_hi), lo_dat)
        ...
        print("AFTER RESAMPLING2", hi_dat.shape, lo_dat.shape, result.shape, result2.shape)

        print("MASKING", np.count_nonzero(result.mask), np.count_nonzero(datLowRes))
