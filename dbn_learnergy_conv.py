...
def setup_ddp(device_ids, use_gpu=True):
    ...
    dist.init_process_group(driver, timeout=timedelta(seconds=5400))#, rank=rank, world_size=world_size, init_method="env://", timeout=timedelta(seconds=10))
...
def run_dbn(yml_conf):
    ...
    if not os.path.exists(model_file) or overwrite_model:
        ...
        while(count == 0 or x2.has_next_subset()):
            mse, pl = new_dbn.fit(x2, batch_size=batch_size, epochs=epochs) #, TODO
               #is_distributed = True, num_loader_workers = num_loader_workers, pin_memory=~use_gpu_pre) #int(os.cpu_count() / 3)) TODO
            count = count + 1 
            x2.next_subset()
...
def generate_output(dat, mdl, use_gpu, out_dir, output_fle, mse_fle, output_subset_count, pin_mem = False):
    ...
    while(count == 0 or dat.has_next_subset() or (dat.subset > 1 and dat.current_subset > (dat.subset-2))):
        ...
        for data in test_loader:
            ...
            print("ERROR", dat.has_next_subset(), ind, ind1, ind2, output.shape, output_full.shape, len(dat_dev)*ind, len(dat_dev)*(ind+1))
