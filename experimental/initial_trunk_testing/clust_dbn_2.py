...
class ClustDBN(Model):
    ...
    def __init__(self, dbn_trunk, input_fc , n_classes, use_gpu=True, scaler = None, layered = False):
        ...
        self.optimizer = torch.optim.AdamW([
            ...
            {'params': self.dbn_trunk.parameters()}],
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #   y = torch.from_numpy((y.cpu().numpy()), dtype = dt)
        #   if hasattr(self.dbn_trunk, "torch_device"):
            ...
            for x_batch, _ in tqdm(batches): 
                ...
                with torch.autocast(device_type=self.device, dtype=dt):
                    ...
                    with torch.no_grad():
                        ...
                        #    y = torch.flatten(torch.as_tensor(y, dtype=dt), start_dim = 1)
                        #    #y2 = torch.flatten(torch.as_tensor(self.scaler.transform(y2), dtype=dt), start_dim = 1)
                        #    y = torch.flatten(torch.as_tensor(y2, dtype=dt), start_dim = 1)
                        #else:
                        ...
                        #    y = torch.flatten(torch.from_numpy(y.cpu().numpy(), dtype=dt), start_dim = 1)
                        #    y2 = torch.flatten(torch.from_numpy(y2.cpu().numpy(), dtype=dt), start_dim = 1)
                ...
                for param in self.fc.parameters():
                    param.grad = None
                for param in self.dbn_trunk.parameters():
                    param.grad = None
