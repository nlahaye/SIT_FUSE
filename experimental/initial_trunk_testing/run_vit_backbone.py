...
def run_dbn(yml_conf):
    ...
    new_dbn = RegionViT(dim = (64, 128, 256, 512), depth = (2, 2, 14, 2), window_size = 7, local_patch_size = 4, num_classes = 1000, tokenize_local_3_conv = False, channels = 3, use_peg = False).cuda()
    new_dbn.load_state_dict(torch.load("/data/nlahaye/output/Learnergy/DBN_eMAS_FULL_STRAT_Segformer_DINO/pretrained-vit_RGB.pt"))
        ...
        with torch.no_grad():
            ...
            for t in range(0, len(data_test)):
                ...
                for frac_ind in range(0,20):
                    ...
                    subset=output_subset_count, tile=tile, tile_size=tile_size, tile_step=tile_step, data_fraction=20, data_fraction_index=frac_ind)
                    x3.scaler = None
...
def generate_output(dat, mdl, use_gpu, out_dir, output_fle, mse_fle, output_subset_count, pin_mem = False, conv = False):
    ...
    while(count == 0 or dat.has_next_subset() or (dat.subset > 1 and dat.current_subset > (dat.subset-2))):
        output_batch_size = min(300, max(int(dat.data_full.shape[0] / 5), dat.data_full.shape[0]))
