...
def run_dbn(yml_conf):
    ...
    new_dbn = RegionViT(dim = (64, 128, 256, 512), depth = (2, 2, 14, 2), window_size = 7, local_patch_size = 4, num_classes = 1000, tokenize_local_3_conv = False, channels = 34, use_peg = False).cuda()
    #new_dbn.load_state_dict(torch.load("/data/nlahaye/output/Learnergy/DBN_eMAS_FULL_STRAT_Segformer_DINO_full_channel/pretrained-vit_RGB.pt"))
  
    print("MODULE LEN", len(list(new_dbn.children())) )
    modules = list(new_dbn.children())[:-5]
    ...
    if auto_clust > 0:
        ...
        visible_shape  = int(200704) #dbn_filters * dbn_hidden[0] * dbn_hidden[1]
        ...
        with torch.no_grad():
            ...
            for t in range(0, len(data_test)):
                ...
                for frac_ind in range(0,100):
                    ...
                    subset=output_subset_count, tile=tile, tile_size=tile_size, tile_step=tile_step, data_fraction=100, data_fraction_index=frac_ind)
                    x3.scaler = None
