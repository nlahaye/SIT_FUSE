...
from learnergy.models.deep_cluster.clust_dbn import ClustDBN
...
def run_dbn(yml_conf):
    ...
    if not os.path.exists(model_file) or overwrite_model:
        ...
        auto_clust = True #TODO parameterize
        if auto_clust == True:
    ...
    if local_rank == 0:
        ...
        for t in range(0, len(data_test)):
            ...
            generate_output(x3, new_dbn, use_gpu, out_dir, "file_clust" + str(t) + "_" +  testing_output, testing_mse, output_subset_count, ~use_gpu_pre)
	...
	#Generate test datasets. Doing it this way, because generating all at once creates memory issues
        ...
        for t in range(0, len(data_train)):
            ...
            generate_output(x2, new_dbn, use_gpu, out_dir, "file_clust" + str(t) + "_" +  training_output, training_mse, output_subset_count, ~use_gpu_pre)
...
def generate_output(dat, mdl, use_gpu, out_dir, output_fle, mse_fle, output_subset_count, pin_mem = False):
    ...
    while(count == 0 or dat.has_next_subset() or (dat.subset > 1 and dat.current_subset > (dat.subset-2))):
        ...
        for data in test_loader:
            ...
            ind1 = len(dat_dev)*ind
            ind2 = len(dat_dev)*(ind+1)
            ind3 = len(dat_dev)
            if ind2 > output_full.shape[0]:
                ind2 = output_full.shape[0]
                ind3 = ind2 - ind1
            print("ERROR", dat.has_next_subset(), ind, ind1, ind2, ind3, output.shape, output_full.shape, len(dat_dev)*ind, len(dat_dev)*(ind+1))
            output_full[ind1:ind2,:] = output[:ind3]
