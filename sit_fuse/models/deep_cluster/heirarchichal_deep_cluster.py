...
class HeirClust(Module):
  
    def __init__(self, base_clust, n_classes, use_gpu=True, min_samples = 1000, gauss_stdevs = [0.01,0.001,0.0], layered = False, train_data=None):
    ...
    def generate_label_set(self, data, use_gpu=True):
        ...
        while(count == 0 or data.has_next_subset() or (data.subset > 1 and data.current_subset > (data.subset-2))):
            ...
            for data2 in tqdm(test_loader):
                dat_dev, lab_dev = data2[0].to(device=self.base_clust.device, non_blocking=True), data2[1].to(device=self.base_clust.device, non_blocking=True)
                ...
                for lab2 in self.clust_tree[lab1].keys(): 
                    if lab2 not in state_dict[lab1].keys():
                        if self.clust_tree[lab1][lab2] is not None:
                            ...
                            uid = str(uuid.uuid1())
        state_dict["labels"] = self.lab_full
        return state_dict  
        ...
        for lab1 in self.clust_tree.keys():
            ...
            for lab2 in self.lab_full.keys():
                ...
                if lab2 in state_dict[lab1].keys():
                    self.clust_tree[lab1][lab2] = MultiPrototypes(self.base_clust.n_classes, self.n_classes, 1)
                    self.clust_tree[lab1][lab2].load_state_dict(state_dict[lab1][lab2]["model"])
        self.lab_full = state_dict["labels"] 
