...
from rbm_models.clust_dbn_2 import ClustDBN
...
def run_dbn(yml_conf):
    ...
    new_dbn = multichannel_resnet.Resnet_multichannel(pretrained=True, encoder_depth=152, num_in_channels=34).cuda()
    #new_dbn.load_state_dict(torch.load("segmentation_backbone_resnet152_state_dict.pt"))

    #new_dbn = torch.load("segmentation_backbone_resnet152.pt").cuda()
    #torch.save(new_dbn.state_dict(), "segmentation_backbone_resnet152_state_dict.pt")

    modules = list(new_dbn.children())[:-2]
    new_dbn = nn.Sequential(*modules)

    new_dbn = DDP(new_dbn, device_ids=[0], broadcast_buffers=False)  
    ...
    if auto_clust > 0:
        ...
        visible_shape  = 2048*2*2 #dbn_filters * dbn_hidden[0] * dbn_hidden[1]
        clust_dbn = ClustDBN(new_dbn, visible_shape, auto_clust, True, clust_scaler)
        clust_dbn.fc = DDP(clust_dbn.fc, device_ids=[local_rank], output_device=local_rank, broadcast_buffers=False)
        final_model = clust_dbn
        if True: #not os.path.exists(model_file + "_fc_clust.ckpt") or overwrite_model:
...
def generate_output(dat, mdl, use_gpu, out_dir, output_fle, mse_fle, output_subset_count, pin_mem = False, conv = False):
    ...
    while(count == 0 or dat.has_next_subset() or (dat.subset > 1 and dat.current_subset > (dat.subset-2))):
        output_batch_size = min(500, max(int(dat.data_full.shape[0] / 5), dat.data_full.shape[0]))
        ...
        for data in tqdm(test_loader):
            ...
            dat_dev = dat_dev.detach()
            lab_dev = lab_dev.detach()
